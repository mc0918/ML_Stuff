...
model.add(
  tf.layers.dense({ units: 512, inputShape: [256], activation: "sigmoid" })
);
model.add(
  tf.layers.dense({ units: 256, inputShape: [512], activation: "sigmoid" })
);
...
AI.prototype.save_data = function(player, computer, ball) {
  ...
  if (this.flip_table) {
    ...
    index =
      width - player.x > this.previous_data[1]
        ? 0
        : width - player.x == this.previous_data[1]
        ? 1
        : 2;
